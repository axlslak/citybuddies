...
{
    internal static class ClientlessGameDataBootstrap
    {
        ...
        public static int Run(string[] args)
        {
            if (args == null || args.Length != 3)
            {
                Console.Error.WriteLine(
                    "Internal usage: Buddies.exe restore-gamedata <cache-directory> <output-directory>");
                return 2;
            }
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    httpClient.DefaultRequestHeaders.UserAgent.ParseAdd(
                        "CityBuddies-GameDataBootstrap/1.0");

                    foreach (RequiredFile requiredFile in RequiredFiles)
                    {
                        string cachePath = Path.Combine(cacheDirectory, requiredFile.Name);
                        string outputPath = Path.Combine(outputDirectory, requiredFile.Name);
        ...
        private static void EnsureCached(
            ...
            string cachePath)
        {
            if (File.Exists(cachePath) && HashMatches(cachePath, requiredFile.Sha256))
                return;

            Console.WriteLine(
                $"Downloading AOSharp.Clientless GameData/{requiredFile.Name}...");

            string temporaryPath = cachePath + ".download-" + Guid.NewGuid().ToString("N");
        ...
        private static void EnsureOutputCopy(
            ...
            string outputPath)
        {
            if (File.Exists(outputPath) && HashMatches(outputPath, requiredFile.Sha256))
                return;
